...
def walk(dude, elephant, valves_opened, total_pressure_released, minutes_left):
	...
	if 0 <= dude[STEPS] and 0 <= elephant[STEPS]:
		# p("Walking...")
		return walk(dude, elephant, valves_opened, total_pressure_released + total_flow, minutes_left-1)
	...
	if dude[STEPS] == -1 and elephant[STEPS] == -1:
		for v1 in valves_closed:
			for v2 in valves_closed:
				...
				# potential_flow = sum([ volcano[v]['flow_rate'] for v in valves_opened ])
				# potential_total = total_pressure_released + potential_flow * minutes_left
				# if potential_total < max_value:
				# 	print("Skipping", minutes_left, potential_total, "<", max_value)
				# 	return total_pressure_released


				dude_, elephant_ = dude[:], elephant[:]
				
				dude_[STEPS] = ROUTING_TABLE[dude[TARGET]][v1]
				dude_[TARGET] = v1
				elephant_[STEPS] = ROUTING_TABLE[elephant[TARGET]][v2]
				elephant_[TARGET] = v2
				...
				r = walk(dude_, elephant_, valves_opened, total_pressure_released + total_flow, minutes_left-1)
		...
		for v1 in valves_closed:
			...
			# potential_total = total_pressure_released + potential_flow * minutes_left
			# if potential_total < max_value:
			# 	# print("Skipping", minutes_left)
			# 	return total_pressure_released

			dude_ = dude[:]
			dude_[STEPS] = ROUTING_TABLE[dude_[TARGET]][v1]
			dude_[TARGET] = v1
			r = walk(dude_, elephant[:], valves_opened, total_pressure_released + total_flow, minutes_left-1)
		...
		for v1 in valves_closed:
			...
			elephant_ = elephant[:]
			elephant_[STEPS] = ROUTING_TABLE[elephant_[TARGET]][v1]
			elephant_[TARGET] = v1
			r = walk(dude[:], elephant_, valves_opened, total_pressure_released + total_flow, minutes_left-1)
...
r = walk(['AA', 0], ['AA', 9999999], set(['AA']), 0, 30)
# r = walk(['AA', 0], ['AA', 0], set(['AA']), 0, 26)
